...
const categoryMarqueeImages = {
  confectionery: [...confectioneryImages, ...confectioneryImages],
  pantry: [...pantryImages, ...pantryImages],
  healthy: [...healthyImages, ...healthyImages],
  fadefit: [],
  burjbites: []
};

const CategoryCard = ({ cat, index, t }) => {
  const cardRef = useScrollReveal(0.1 + (index * 0.1));

  return (
    <Link to={`/category/${cat.route}`} ref={cardRef} className="reveal group relative h-[500px] overflow-hidden bg-navy cursor-pointer rounded-sm shadow-sm block">
      {/* Background Image */}
      <div 
      ...
      <div className="absolute inset-0 p-8 flex flex-col justify-end">
        ...
        <div>
          ...
          {cat.marquees && cat.marquees.length > 0 ? (
          <div
          ...
          >
            ...
            <div className="text-white/60 text-sm italic">Images coming soon</div>
          )}
        </div>
      </div>
    </Link>
...
const Categories = () => {
  ...
  const exportCategories = [
    { id: 'CAT-01', route: 'confectionery', name: t.categories.confectionery, image: categoryImages[0], marquees: categoryMarqueeImages.confectionery },
    { id: 'CAT-02', route: 'pantry', name: t.categories.pantry, image: categoryImages[1], marquees: categoryMarqueeImages.pantry },
    { id: 'CAT-03', route: 'healthy', name: t.categories.healthy, image: categoryImages[2], marquees: categoryMarqueeImages.healthy },
  ];

  const importCategories = [
    { id: 'IMP-01', route: 'fadefit', name: t.categories.fadefit, image: 'https://images.unsplash.com/photo-1603569283847-aa295f0d016a?q=80&w=2000&auto=format&fit=crop', marquees: categoryMarqueeImages.fadefit },
    { id: 'IMP-02', route: 'burjbites', name: t.categories.burjbites, image: 'https://images.unsplash.com/photo-1581006509951-e12484a929fb?q=80&w=2000&auto=format&fit=crop', marquees: categoryMarqueeImages.burjbites },
  ];
    ...
    <section className="py-24 bg-off2" id="categories">
      <div className="container mx-auto px-8">
        ...
        <div className="flex justify-center mb-12">
          <div className="inline-flex bg-navy/5 p-1 rounded-full">
            <button
              onClick={() => setActiveTab('export')}
              className={`px-8 py-3 rounded-full text-sm font-bold tracking-widest uppercase transition-all duration-300 ${activeTab === 'export' ? 'bg-green text-white shadow-md' : 'text-navy hover:bg-navy/10'}`}
            >
              {t.categories.exportTab}
            </button>
            <button
              onClick={() => setActiveTab('import')}
              className={`px-8 py-3 rounded-full text-sm font-bold tracking-widest uppercase transition-all duration-300 ${activeTab === 'import' ? 'bg-green text-white shadow-md' : 'text-navy hover:bg-navy/10'}`}
            >
              {t.categories.importTab}
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-center">
          {categoriesToShow.map((cat, index) => (
            <CategoryCard key={cat.id} cat={cat} index={index} t={t} />
          ))}
        </div>
